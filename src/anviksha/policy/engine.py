...
class PolicyEngine:
    def __init__(self, policies: tuple[Policy, ...] = ()) -> None: self._policies = policies
    def validate_plan(self, request: ExecutionRequest, plan: ExecutionPlan) -> None:
        for policy in self._policies: policy.validate_plan(request, plan)
    def validate_response(self, request: ExecutionRequest, response: RuntimeResponse) -> None:
        for policy in self._policies: policy.validate_response(request, response)

class MinimumConfidencePolicy:
    name = "minimum_confidence"
    def validate_plan(self, request: ExecutionRequest, plan: ExecutionPlan) -> None: return None
    def validate_response(self, request: ExecutionRequest, response: RuntimeResponse) -> None:
        if response.confidence < request.constraints.min_confidence:
            raise PolicyViolationError(f"confidence {response.confidence:.2f} below required {request.constraints.min_confidence:.2f}")
